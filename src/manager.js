...
const { pathfinder, Movements } = pathfinderPkg; // ✅ FIX CommonJS
...
export class BotsManager {
  constructor() {
    ...
    this.stagger = parseInt(process.env.STAGGER_JOIN_MS || "8000", 10);

    this.reconnectBase = parseInt(process.env.RECONNECT_BASE_MS || "15000", 10);
  ...
  async spawn(slot, state = null) {
    ...
    const bot = mineflayer.createBot({
      ...
      username,
      version: process.env.MC_VERSION || "1.21.9",
      auth: "offline"
    });
    ...
    bot.once("spawn", async () => {
      logger.info(`[Bot ${slot}] spawn OK: ${username}`);
      ...
      try {
        ...
        logger.warn(`[Bot ${slot}] No se pudo cargar minecraft-data/movements: ${e?.message || e}`);
      }
      ...
      bot.chat(cmd);
      logger.info(`[Bot ${slot}] login enviado`);
