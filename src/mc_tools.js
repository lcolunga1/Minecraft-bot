...
const { goals } = pathfinderPkg; // ✅ FIX CommonJS
...
export function createToolset(bot) {
  return {
    ...
    async say({ text }) {
      const max = Number(process.env.CHAT_MAX_CHARS || 120);
      bot.chat(String(text).slice(0, max));
    },
    ...
    async eat_if_needed() {
      ...
      const items = bot.inventory.items();
      const food =
        items.find((i) => i.name.includes("bread")) ||
    ...
    async mine_nearby({ blockName, maxDistance = 16, count = 4 }) {
      ...
      while (mined < count) {
        ...
        bot.pathfinder.setGoal(new goals.GoalNear(b.position.x, b.position.y, b.position.z, 2));
        await sleep(600); // deja que llegue
    ...
    // CRAFT (super básico: planks -> sticks -> crafting table)
    async craft_basic_start() {
      ...
      const haveLogs = bot.inventory
        .items()
        .find((i) => i.name.endsWith("_log") || i.name.endsWith("_stem"));
      if (!haveLogs) return;
      ...
      if (!planksId || !stickId || !tableId) return;

      const plankRecipe = bot.recipesFor(planksId, null, 1, null)[0];
      if (plankRecipe) await bot.craft(plankRecipe, 1, null);

      const stickRecipe = bot.recipesFor(stickId, null, 1, null)[0];
      if (stickRecipe) await bot.craft(stickRecipe, 1, null);

      const ctRecipe = bot.recipesFor(tableId, null, 1, null)[0];
    ...
    async inventory_summary() {
      ...
      return {
        ...
        swords: ["wooden_sword", "stone_sword", "iron_sword", "diamond_sword", "netherite_sword"].filter(
          (n) => top[n]
        )
      };
    }
